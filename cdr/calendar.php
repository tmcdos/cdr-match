...
function getCalendar()
{
	tbl = getEl('pole');
	<?php if($_GET['calo']!='') { ?>
	cal_obj = window.parent.<?php echo $_GET['calo']; ?>;
	if(!cal_obj)
	{
		alert('"<?php echo $_GET['calo']; ?>" not found');
		return;
	}
	...
	if (cal_obj.displayType=="date" || cal_obj.displayType=="week-end") 
	{
		...
		result += '</TR>\n';
		result += '<TR><TD COLSPAN=7 BGCOLOR=#663333 ALIGN=CENTER><IMG SRC="graypixel.gif" WIDTH=120 HEIGHT=1></TD></TR>\n';
		...
		result += '<TR><TD COLSPAN=7 ALIGN=CENTER BGCOLOR=#663333><IMG SRC="graypixel.gif" WIDTH=120 HEIGHT=1></TD></TR>\n';
		result += '<TR>\n';
	...
	if (cal_obj.displayType=="month" || cal_obj.displayType=="quarter" || cal_obj.displayType=="year") 
	{
		...
		{
			result += '<TABLE WIDTH=154 BORDER=0 BORDERWIDTH=0 CELLSPACING=0 CELLPADDING=0 style="font-family:Tahoma; font-size:8pt">\n';
			result += '<TR BGCOLOR="#C0C0C0">\n';
			result += '	<TD BGCOLOR="#C0C0C0" CLASS="cal" WIDTH=25 ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="javascript:;" onClick="'+refresh+"('"+pole+"',"+(year-1)+');">&lt;&lt;</A></B></TD>\n';
			result += '	<TD BGCOLOR="#C0C0C0" CLASS="cal" WIDTH=104 ALIGN=CENTER><b>'+year+'</b></TD>\n';
			result += '	<TD BGCOLOR="#C0C0C0" CLASS="cal" WIDTH=25 ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="javascript:;" onClick="'+refresh+"('"+pole+"',"+(year+1)+');">&gt;&gt;</A></B></TD>\n';
			result += '</TR></TABLE>\n';
		}
	}
	...
	if (cal_obj.displayType=="month") 
	{
		...
		result += '<TABLE WIDTH=120 BORDER=0 CELLSPACING=2 CELLPADDING=2 ALIGN=CENTER style="font-family:Tahoma; font-size:8pt">\n';
		for (var i=0; i<4; i++) 
		{
			result += '<TR>';
			for (var j=0; j<3; j++) 
